// app/gallery/page.tsx
import Image from 'next/image';
import BookAndPayButton from '../components/BookAndPayButton';
import { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Before & After Gallery | Smile Transformations – Dental Concepts Delhi',
  description:
    'Real patient smile makeovers at Dental Concepts Delhi. See amazing results of Dental Implants, Veneers, Braces, Root Canal & Teeth Whitening in Hari Nagar.',
};

export default function GalleryPage() {
  return (
    <>
      {/* HERO */}
      <section className="bg-gradient-to-br from-teal-50 to-white py-20 lg:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl lg:text-6xl font-bold text-gray-900 mb-6 leading-tight">
            Real Smiles, Real Results
          </h1>
          <p className="text-xl text-gray-700 max-w-4xl mx-auto mb-10">
            See what our patients looked like <span className="font-bold text-teal-600">before</span> and how they smile{' '}
            <span className="font-bold text-teal-600">after</span> treatment at Dental Concepts Delhi.
          </p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <BookAndPayButton />
            <a
              href="https://wa.me/919876543210?text=Hi! I saw your before-after gallery. I want the same smile!"
              className="bg-white text-teal-600 font-bold py-5 px-12 rounded-full text-xl hover:bg-gray-100 transition inline-flex items-center gap-3"
            >
              <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.626.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488z" />
              </svg>
              Get My Dream Smile
            </a>
          </div>
        </div>
      </section>

      {/* BEFORE & AFTER GRID */}
      <section className="section-padding bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[
              {
                treatment: 'Full Smile Makeover (8 Veneers)',
                patient: 'Ria, 28',
                before: '/images/before-1.jpg',
                after: '/images/after-1.jpg',
              },
              {
                treatment: 'Dental Implants (Front Teeth)',
                patient: 'Mr. Kapoor, 52',
                before: '/images/before-2.jpg',
                after: '/images/after-2.jpg',
              },
              {
                treatment: 'Clear Aligners (18 months)',
                patient: 'Ananya, 19',
                before: '/images/before-3.jpg',
                after: '/images/after-3.jpg',
              },
              {
                treatment: 'Teeth Whitening + Cleaning',
                patient: 'Neha, 34',
                before: '/images/before-4.jpg',
                after: '/images/after-4.jpg',
              },
              {
                treatment: 'Root Canal + Crown',
                patient: 'Rohit, 41',
                before: '/images/before-5.jpg',
                after: '/images/after-5.jpg',
              },
              {
                treatment: 'Kids Braces (Age 12)',
                patient: 'Aarav, 12',
                before: '/images/before-6.jpg',
                after: '/images/after-6.jpg',
              },
            ].map((item, i) => (
              <div
                key={i}
                className="bg-[#faf9f6] rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition transform hover:-translate-y-4"
              >
                <div className="grid grid-cols-2">
                  {/* Before */}
                  <div className="relative">
                    <Image
                      src={item.before}
                      alt={`Before – ${item.treatment} – ${item.patient}`}
                      width={600}
                      height={600}
                      className="object-cover w-full h-full"
                      placeholder="blur"
                      blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8/OnPfQAJJAPr8gP4OQAAAABJRU5ErkJggg=="
                    />
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <span className="text-white font-bold text-3xl">Before</span>
                    </div>
                  </div>

                  {/* After */}
                  <div className="relative">
                    <Image
                      src={item.after}
                      alt={`After – ${item.treatment} – ${item.patient}`}
                      width={600}
                      height={600}
                      className="object-cover w-full h-full"
                      placeholder="blur"
                      blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN8/OnPfQAJJAPr8gP4OQAAAABJRU5ErkJggg=="
                    />
                    <div className="absolute inset-0 bg-teal-600/80 flex items-center justify-center">
                      <span className="text-white font-bold text-3xl">After</span>
                    </div>
                  </div>
                </div>

                {/* Caption */}
                <div className="p-6 text-center">
                  <h3 className="text-xl font-bold text-gray-900 mb-1">{item.treatment}</h3>
                  <p className="text-teal-600 font-medium">{item.patient}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* FINAL CTA */}
      <section className="bg-teal-600 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6">
            Your Dream Smile Starts Today!
          </h2>
          <p className="text-xl text-teal-50 mb-10">
            Book your consultation now – same-day appointments available
          </p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <BookAndPayButton />
            <a
              href="tel:+919876543210"
              className="bg-white text-teal-600 font-bold py-5 px-12 rounded-full text-xl hover:bg-gray-100 transition"
            >
              Call Now: +91 98765 43210
            </a>
          </div>
        </div>
      </section>
    </>
  );
}